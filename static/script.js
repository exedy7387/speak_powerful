...
const BUTTON_COUNT_KEY = "visiblePlayButtonCount";
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    applyButtonCount(getSavedButtonCount());
...
async function buildMediaConfigUI() {
    ...
    countRow.className = "popup-row";

    const countLabel = document.createElement("label");
    countLabel.textContent = "表示ボタン数(1〜12)";

    const countInput = document.createElement("input");
    countInput.type = "number";
    countInput.min = "1";
    countInput.max = "12";
    countInput.value = getSavedButtonCount();
    countInput.id = "playButtonCountInput";
...
btnSave.addEventListener("click", () => {
    ...
    // ★ 追加：表示ボタン数 保存＆反映
    const countInput = document.getElementById("playButtonCountInput");
    const count = countInput ? countInput.value : 12;
    localStorage.setItem(BUTTON_COUNT_KEY, String(count));
    applyButtonCount(count);

    closePopup();
});

function applyButtonCount(count) {
    const n = Math.max(1, Math.min(12, Number(count) || 12));
    const buttons = document.querySelectorAll(".play-button");
    buttons.forEach((btn, i) => {
        btn.style.display = i < n ? "block" : "none";
    });
}

function getSavedButtonCount() {
    return Number(localStorage.getItem(BUTTON_COUNT_KEY) || 12);
}
